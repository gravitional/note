...
+ 释放: delete[] 数组名 p
    ...
    不仅要有数组名, 还要有方括号. 如果没有方括号, 只释放数组首元素的地址.
...
手动 `new/delete` 具有安全隐患. 在 `C++ 11` 中提供了智能指针.
...
+ 输入字符串时, 可以使用其它分隔符作为字符串结束的标志(例如逗号, 分号), 将分隔符作为 getline 的第3参数即可. 例如:
