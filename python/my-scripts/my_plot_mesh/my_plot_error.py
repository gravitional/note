...
def plot_error(nodeData: ndarray, Velements: ndarray, errArray: ndarray):
    ...
    nodeN = Velements.shape[1] - VnodeOFST  # ele 拥有的节点数
    Vtriangles = np.empty((errLen, nodeN), dtype=int)  # 体单元节点构成
    eleErrs = np.empty(errLen, dtype=float)  # 误差向量
    # print(f'ele array: {errArray}')
    ii = 0
    xlst = nodeData[:, nodePosX]
    ylst = nodeData[:, nodePosY]
    for errPair in errArray:
        eleID = int(errPair[0])
        Vtriangles[ii] = Velements[eleID, VnodeOFST:]
        eleErrs[ii] = errPair[1] * _error_norm
        ii += 1
