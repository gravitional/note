...
+ `( )` ;  标记子表达式的开始和结束位置. 子表达式可以获取供以后使用. 要匹配字面的 `()`, 请使用 `\(` 和 `\)`.
...
1. 非贪婪模式尽可能少的匹配所搜索的字符串, 而默认的贪婪模式则尽可能多的匹配所搜索的字符串.
例如, 对于字符串 `oooo`, `o+?`将匹配单个`o`, 而 `o+`将匹配所有`o`.

`*`, `+` 限定符都是贪婪的, 因为它们会尽可能多的匹配文字,
只要在它们的后面加上 `?` 就可以实现非贪婪或最小匹配.
...
+ `\` ; 将下个字符标记为特殊字符, 或原义字符, 或向后引用, 或八进制转义符.
例如, `n` 匹配字符 `n`. `\n` 匹配换行符. 序列 `\\` 匹配 `\` 而 `\(` 则匹配 `(`.
...
+ `{n}` ; `n` 是非负整数. 匹配确定的 n 次. 例如, `o{2}` 不能匹配 `Bob` 中的 `o`, 但是能匹配 `food` 中的两个 o.
+ `{n,}` ; `n` 是非负整数. 至少匹配 `n` 次.
...
+ `(?:pattern)` ; 匹配 pattern 但不获取匹配结果, 也就是说这是`非获取匹配`, 不进行存储供以后使用.
这在使用 `或` 字符 `(|)` 来组合模式的各个部分是很有用.
...
+ `xy`     | ; 匹配 x 或 y. 例如, `z|food` 能匹配 `z` 或 `food`. `(z|f)ood` 则匹配 `zood` 或 `food`.
