...
+ ParallelSubmit[cmd]将cmd提交给远程内核进行计算, 并返回队列中作业的eid.
+ ParallelSubmit[{vars ...},cmd] 在将cmd发送到远程内核之前, 为给定变量的本地值建立一个闭包.
+ WaitAll[eid] 等待给定的进程结束并返回其结果.
+ WaitAll[{Subscript[eid, 1],Subscript[eid, 2],...}] 等待所有给定的进程并返回结果列表.
+ WaitAll[expr]等待expr中包含的所有计算ID完成, 并以各自进程的结果取代它们.
+ WaitNext[{Subscript[eid, 1],Subscript[eid, 2],...}] 等待其中一个给定的进程完成.
它返回{res,id,ids}, 其中id是已完成进程的eid, res是其结果, ids是剩余eid的列表.

`WaitNext` 是不确定的.它返回任意一个已经结束的进程.
