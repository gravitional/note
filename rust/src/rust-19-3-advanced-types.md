...
正如你可能猜到的, continue 的值是 !. 也就是说, 当 Rust 要计算 guess 的类型时, 它查看这两个分支.
...
所以虽然 `&T` 是单个值, 它储存了 `T` 所在的内存位置; 但 `&str` 则是 **两个** 值: `str` 的地址和其长度.
...
因为其类型可能不是 `Sized` 的, 所以需要将其置于某种指针背后. 在这个例子中我们选择了`引用`.
