...
该附录包含了 Rust 语法的词汇表, 包括运算符以及其他的符号,
这些符号单独出现或出现在路径, 泛型, trait bounds, 宏, 属性, 注释, 元组以及大括号上下文中.

### 运算符

表 B-1 包含了 Rust 中的运算符, 运算符如何出现在上下文中的示例, 简短解释以及该运算符是否可重载.
如果一个运算符是可重载的, 则该运算符上用于重载的相关 trait 也会列出.
