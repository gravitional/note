...
[fpath,~]=fileparts(mfpath);
cd(fpath)

off_c2ML=1; % C++数组从0开始，MatLab 从1开始
%电磁常数
er0 = 8.854187817e-12;
mu0 = 4*pi*1e-7;
penalty=1e10;%罚系数
...
PlaneNdID=PlaneNdID+off_c2ML; % 编号偏移
off_plane=1;%单元的节点数据在 2 3 4列, 偏移量=1
off_surface=1;% surface 单元中节点列的偏移, 从第2行开始
%++++SurfaceNdID: 线单元编号 节点1ID 节点2ID 线所属单元ID 罚系数
SurfaceNdID = readmatrix('Surface.txt');% 边界单元
SurfaceNdID=SurfaceNdID+off_c2ML; % 编号偏移

%++++ 将单元划分到不同的 Region
region(1,:)=[0 3121]+off_c2ML;%外部区域
region(2,:)=[3122 3479]+off_c2ML;%外部区域

%++++第一类边界条件上的单元，准备罚系数
voltRefID(1,:)=[3480 3519]+off_c2ML;%第二组边界
voltRefID(2,:)=[3520 3559]+off_c2ML;%第一组边界

%++++介电屏蔽条件所在的 surface ID
screenRID=[3560 3605]+off_c2ML;
...
for ne=1:PlaneSize(1)
    for np=1:3
        nodeID=PlaneNdID(ne,np+off_plane); %节点ID
...
for nl = 1:SurfaceSize(1)
    for np = 1:2
        refID=SurfaceNdID(nl,np+off_surface);
...
%对三角形单元做循环, 计算电磁场强度
for ne = 1:length(PlaneNdID)
    for np = 1:3
        refID=PlaneNdID(ne,np+off_plane);
