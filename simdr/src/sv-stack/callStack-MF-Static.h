    ...
    common::Control().Execute(argc, argv); // 总流程控制类, 管理 Impl 资源, /Common/Control/Control.cpp
        ...
        _pIml->CreateModel(); //创建模型
            ...
            creator->Create(); // 创建网格和模型, /Common/Model/ModelCreator.cpp
                SetFieldInfo(); //设置场量的 量纲 单位
                CreateMesh(); // 创建网格
                CreateModel(); //创建模型 /FiniteElement/Model/FeModelCreator.cpp
                    CreateFunction(); //nv; 自定义函数
                    CreateCoordSys(); //nv; 自定义坐标系
                    CreateMonitor();  //nv;创建监视
                    CreateNode(); // nv; 创建节点
                        CompNode* node=NewNode(); //节点工厂方法, 调用具体实现
                    CreateElement(data);  //nv;创建单元
                    CreateMaterial(data); //nv; 创建材料
                    CreateMisc(data); //vt; 创建各场的特有数据, /Current/Model/ModelCreatorCF.cpp
                        modelMF()->Initialize(); // eddy effect; ele on ConsSurf; node on ConsSurf; node edge sign;
                        CreateInfo(data); // 创建全局参数
                            data.ReadValue("Thickness2D"...) //读取厚度
                            data.ReadValue("Sector"...) //读取模型分数
                        CreateNetlist(data); // 创建 net list
                        CreateEddy(data); // 创建 eddy 效应;
                            eddy->Create(data);
                                SetEddy(theID); // 设置 Eddy 效应
                                    modelMF()->SetEddyEffect(id); // 设置 eleID eddyEffect 为 true
                        CreateMotion(data); // 创建 运动;
                        CreateConnectedCoils(data);// 创建连接线圈
                        CreateConstraint(data) // 创建约束, 即边界条件,
                        // Vector, Open, IBC, Periodic, NonConform2D, NonConform3D
                    CreateInitialField(data); //vt; 创建初始场
                    CreateAnalysis(); //vt; 创建分析, /FiniteElement/Model/FeModelCreator.cpp
                        //迭代分析类型
                    ...
                    Initialize(); // 各个场自己的特定初始化, /Current/Model/ModelCreatorCF.cpp
                        ...
                        ApplyConstraint(); // 施加约束
                            cons->ExpandCons2Ele();//施加约束到单元上
                            eddy->EddyEffect2FEM(); // Eddy 效应
                            coil->InitialCoils(); // 初始化 线圈
                            mpc->BuildMstSlvDofConnection();// 建立主从约束; 多点共自由度
                            edge->Restrain(0); // MPI edge restrain
                        ArrangeEquationNo(); // 数据自由度排序
                        MPIRenumber(); // MPI 重新排序
