...
6. `..` : 创建一个范围闭区间
7. `-f` : 格式化数据
8. `$` : 将字符串内部的变量转换为实际的值; 结合双引号实现字符串内插; `"var is $var"`
9. `@()` : 将一系列值转换为一个数组
10. `,` : 数组分隔,或创建单元素数组
11. `#` : 添加注释,单行
...
如果该字符串是一个`命令`或者`外部程序`,在字符串前加"`&`"可以执行命令,或者启动程序.
...
和其他语言一样, `(...)` 在表达式中用于覆盖运算符优先级. 例如. `(1+2)/3`.
然而, 在`PowerShell`中, 还有一些额外的行为.
...
+ `@()`运算符用于将一系列值转换为一个数组.
假如在脚本中有一个函数可能返回`0`, `1`或多个值,
就可以使用这个操作符,将一系列值合并为一个数组,方便后续处理.
